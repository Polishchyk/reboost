  ...
  <div v-if="devicePageData?.data">
      ...
      <div class="wrap">
        <div class="nav">
          <nuxt-link :to="locale !== defaultLocale ? `/${locale}` : '/'">Home</nuxt-link>
            <nuxt-link :to="`${locale !== defaultLocale ? `/${locale}/` : `/`}computer-repair`">{{ pageTitles?.['computer-repair'] }}</nuxt-link>
    ...
    <div class="sect-repair-selection">
      <div class="wrap">
        ...
        <h2>{{devicePageData?.data?.hardware?.Title}}</h2>
        ...
        <h2>{{devicePageData?.data?.Software?.Title}}</h2>
